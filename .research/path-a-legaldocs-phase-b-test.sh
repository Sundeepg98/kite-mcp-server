#!/usr/bin/env bash
# Path A.5 — Phase B viability test for kc/legaldocs in scratch.
# Mirrors path-a-i18n-phase-b-test.sh.

set -euo pipefail

SCRATCH=/tmp/legaldocs-phase-b-test
...
echo "Step 2a: drop replace algo2go/kite-mcp-legaldocs in root go.mod"
...
echo ""
echo "Step 2c: drop ./kc/legaldocs from go.work (CRLF-safe \\b)"
sed -i '/^[[:space:]]\+\.\/kc\/legaldocs\b/d' go.work
echo "go.work member count: $(grep -cE '^[[:space:]]+\./' go.work)"

echo ""
echo "Step 2d: rename ./kc/legaldocs out of the way (test-only)"
mv kc/legaldocs kc/legaldocs.MOVED-FOR-PHASE-B-TEST

echo ""
echo "Step 2e: drop kc/legaldocs COPY line from Dockerfile"
...
$GO build ./... 2>&1 | tee /tmp/_ld-pb-build.txt | tail -10
echo ""
echo "--- GOWORK=off go build ./... ---"
GOWORK=off $GO build ./... 2>&1 | tee /tmp/_ld-pb-build-off.txt | tail -10
