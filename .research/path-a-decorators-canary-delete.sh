#!/usr/bin/env bash
# Path A — kc/decorators Phase B canary deletion ON MASTER.
# Mirror of path-b-resume-canary-delete.sh but for decorators alone.
# Drops the canary replace + ./kc/decorators/, fetches from algo2go.
...
find . -path ./kc/decorators -prune -o -path ./.research -prune -o -name 'go.mod' -type f -print | while read -r mod; do
	if grep -qE 'github\.com/algo2go/kite-mcp-decorators => ' "$mod" 2>/dev/null; then
...
echo ""
echo "=== Phase 3: Drop ./kc/decorators from go.work (CRLF-safe) ==="
sed -i '/^[[:space:]]\+\.\/kc\/decorators\b/d' go.work
echo "go.work member count: $(grep -cE '^[[:space:]]+\./' go.work)"

echo ""
echo "=== Phase 4: Drop kc/decorators COPY line from Dockerfile ==="
sed -i '/^COPY kc\/decorators\/go\.mod kc\/decorators\/go\.sum/d' Dockerfile
echo "Dockerfile decorators lines: $(grep -c 'kc/decorators' Dockerfile || true)"

echo ""
echo "=== Phase 5: Delete ./kc/decorators/ ==="
git rm -rq kc/decorators/
...
find . -path ./.research -prune -o -name 'go.mod' -type f -print | while read -r mod; do
	if grep -qE 'github\.com/algo2go/kite-mcp-decorators v0\.0\.0-00010101000000-000000000000' "$mod" 2>/dev/null; then
