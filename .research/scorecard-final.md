...
| 7. Plugin | 99.5 | **100** | +0.5 | **Brief is correct on BOTH axes**: (a) in-process — `94fe4b8` drains 103 tools to `RegisterInternalTool` registry (Investment J complete), **114 `RegisterInternalTool(&...)` call sites** verified at HEAD, universal self-registration across the tool surface; (b) cross-language — `kc/riskguard/checkrpc/` subprocess RPC contract canonicalised (ADR 0007 + README + 6 wire-contract regression tests in `types_test.go`) per the `e84a8f4` language-fit evaluation that established subprocess RPC as the right cross-language plugin vehicle (vs WASM cold-start, vs Go's `plugin.Open` Linux-only fragility). The Windows `dlopen` irreducible from prior baseline is moot under EITHER pattern. | None — capped. Both first-party Go plugins (in-process registry) and cross-language plugins (subprocess RPC, any language speaking netRPC over stdio) have canonical, documented, regression-tested patterns. |
...
| 11. Portability | 84 | **86** | +2 | Phase 2 Fx adoption adds a typed provider graph that — per the language-swap analysis (`a03694a`) — modestly eases per-component lifecycle lift for any subprocess-RPC swap (Rust riskguard, Python analytics). **Plus**: ADR 0007 + `kc/riskguard/checkrpc/` README canonicalises the cross-language plugin RPC contract (the third of the three Axis-C-helping investments named in `a03694a` §4.3 — explicitly closed). Real per-component portability lift requires the actual swap or its remaining scaffolding (Bun/esbuild widget pipeline). +2 reflects: scaffolding-readiness (Phase 2) + the cross-language plugin contract being formal-with-regression-tests (ADR 0007), still not realized portability via a shipped non-Go plugin. | +14 SCALE-GATED (Postgres adapter, Helm/compose, real per-component swap shipped — non-Go riskguard Check, Python analytics kernel, etc.) |
