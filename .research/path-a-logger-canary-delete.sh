#!/usr/bin/env bash
# Path A.7 — kc/logger Phase B canary deletion ON MASTER.
# NOTE: pipefail interaction in find loops (lesson from A.6) — guard with
# `set -uo pipefail` (no -e) for the find-loop sections to allow grep -q
# to return 1 (no match) without aborting.
...
find . -path ./kc/logger -prune -o -path ./.research -prune -o -name 'go.mod' -type f -print | while read -r mod; do
	if grep -qE 'github\.com/algo2go/kite-mcp-logger => ' "$mod" 2>/dev/null; then
...
echo ""
echo "=== Phase 3: Drop ./kc/logger from go.work (CRLF-safe) ==="
sed -i '/^[[:space:]]\+\.\/kc\/logger\b/d' go.work
echo "go.work member count: $(grep -cE '^[[:space:]]+\./' go.work)"

echo ""
echo "=== Phase 4: Drop kc/logger COPY line from Dockerfile ==="
sed -i '/^COPY kc\/logger\/go\.mod kc\/logger\/go\.sum/d' Dockerfile
echo "Dockerfile logger lines: $(grep -c 'kc/logger' Dockerfile || true)"

echo ""
echo "=== Phase 5: Delete ./kc/logger/ ==="
git rm -rq kc/logger/
...
find . -path ./.research -prune -o -name 'go.mod' -type f -print | while read -r mod; do
	if grep -qE 'github\.com/algo2go/kite-mcp-logger v0\.0\.0-00010101000000-000000000000' "$mod" 2>/dev/null; then
