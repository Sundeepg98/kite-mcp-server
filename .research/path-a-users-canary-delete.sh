#!/usr/bin/env bash
# Path A.12 — kc/users Phase B canary deletion ON MASTER.
...
sed -i '/github\.com\/algo2go\/kite-mcp-users => \.\/kc\/users/d' go.mod
...
find . -path ./kc/users -prune -o -path ./.research -prune -o -name 'go.mod' -type f -print | while read -r mod; do
	if grep -qE 'github\.com/algo2go/kite-mcp-users => ' "$mod" 2>/dev/null; then
		sed -i '/github\.com\/algo2go\/kite-mcp-users => /d' "$mod"
		echo "  patched: $mod"
	fi
done

echo ""
echo "=== Phase 3: Drop ./kc/users from go.work (CRLF-safe) ==="
sed -i '/^[[:space:]]\+\.\/kc\/users\b/d' go.work
echo "go.work member count: $(grep -cE '^[[:space:]]+\./' go.work)"

echo ""
echo "=== Phase 4: Drop kc/users COPY line from Dockerfile ==="
sed -i '/^COPY kc\/users\/go\.mod kc\/users\/go\.sum/d' Dockerfile

echo ""
echo "=== Phase 5: Delete ./kc/users/ ==="
git rm -rq kc/users/
...
find . -path ./.research -prune -o -name 'go.mod' -type f -print | while read -r mod; do
	if grep -qE 'github\.com/algo2go/kite-mcp-users v0\.0\.0-00010101000000-000000000000' "$mod" 2>/dev/null; then
