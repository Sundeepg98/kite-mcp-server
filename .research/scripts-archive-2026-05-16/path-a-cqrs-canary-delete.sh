...
sed -i '/github\.com\/algo2go\/kite-mcp-cqrs => \.\/kc\/cqrs/d' go.mod

find . -path ./kc/cqrs -prune -o -path ./.research -prune -o -name 'go.mod' -type f -print | while read -r mod; do
	if grep -qE 'github\.com/algo2go/kite-mcp-cqrs => ' "$mod" 2>/dev/null; then
		sed -i '/github\.com\/algo2go\/kite-mcp-cqrs => /d' "$mod"
		echo "  patched: $mod"
	fi
done

sed -i '/^[[:space:]]\+\.\/kc\/cqrs\b/d' go.work
echo "go.work members: $(grep -cE '^[[:space:]]+\./' go.work)"

sed -i '/^COPY kc\/cqrs\/go\.mod kc\/cqrs\/go\.sum/d' Dockerfile

git rm -rq kc/cqrs/

find . -path ./.research -prune -o -name 'go.mod' -type f -print | while read -r mod; do
	if grep -qE 'github\.com/algo2go/kite-mcp-cqrs v0\.0\.0-00010101000000-000000000000' "$mod" 2>/dev/null; then
