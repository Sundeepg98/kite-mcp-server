#!/usr/bin/env bash
# Path A.10 — kc/domain Phase B canary deletion ON MASTER.
...
sed -i '/github\.com\/algo2go\/kite-mcp-domain => \.\/kc\/domain/d' go.mod
grep -nE 'algo2go/kite-mcp-domain' go.mod | head -3
...
find . -path ./kc/domain -prune -o -path ./.research -prune -o -name 'go.mod' -type f -print | while read -r mod; do
	if grep -qE 'github\.com/algo2go/kite-mcp-domain => ' "$mod" 2>/dev/null; then
		sed -i '/github\.com\/algo2go\/kite-mcp-domain => /d' "$mod"
		echo "  patched: $mod"
	fi
done

echo ""
echo "=== Phase 3: Drop ./kc/domain from go.work (CRLF-safe) ==="
sed -i '/^[[:space:]]\+\.\/kc\/domain\b/d' go.work
echo "go.work member count: $(grep -cE '^[[:space:]]+\./' go.work)"

echo ""
echo "=== Phase 4: Drop kc/domain COPY line from Dockerfile ==="
sed -i '/^COPY kc\/domain\/go\.mod kc\/domain\/go\.sum/d' Dockerfile
echo "Dockerfile domain lines: $(grep -c 'kc/domain' Dockerfile || true)"

echo ""
echo "=== Phase 5: Delete ./kc/domain/ ==="
git rm -rq kc/domain/
...
find . -path ./.research -prune -o -name 'go.mod' -type f -print | while read -r mod; do
	if grep -qE 'github\.com/algo2go/kite-mcp-domain v0\.0\.0-00010101000000-000000000000' "$mod" 2>/dev/null; then
