...
find . -path ./kc/papertrading -prune -o -path ./.research -prune -o -name 'go.mod' -type f -print | while read -r mod; do
	if grep -qE 'github\.com/algo2go/kite-mcp-papertrading => ' "$mod" 2>/dev/null; then
...
git rm -rq kc/papertrading/

find . -path ./.research -prune -o -name 'go.mod' -type f -print | while read -r mod; do
	if grep -qE 'github\.com/algo2go/kite-mcp-papertrading v0\.0\.0-00010101000000-000000000000' "$mod" 2>/dev/null; then
