...
find . -path ./kc/riskguard -prune -o -path ./.research -prune -o -name 'go.mod' -type f -print | while read -r mod; do
	if grep -qE 'github\.com/algo2go/kite-mcp-riskguard => ' "$mod" 2>/dev/null; then
...
git rm -rq kc/riskguard/

find . -path ./.research -prune -o -name 'go.mod' -type f -print | while read -r mod; do
	if grep -qE 'github\.com/algo2go/kite-mcp-riskguard v0\.0\.0-00010101000000-000000000000' "$mod" 2>/dev/null; then
