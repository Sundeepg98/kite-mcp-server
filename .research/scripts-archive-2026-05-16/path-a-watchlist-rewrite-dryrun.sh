#!/usr/bin/env bash
# Path A.15 — kc/watchlist rewrite dry-run.
set -euo pipefail
SCRATCH=/tmp/algo2go-watchlist-extract-dryrun/kite-mcp-watchlist-extract
[ -d "$SCRATCH" ] || { echo "ERROR: run prep first"; exit 1; }
cd "$SCRATCH"

echo "=== Phase 9: Rewrite kc/watchlist self-imports + go.mod module path ==="
find . -name '*.go' -type f -exec sed -i 's#github.com/zerodha/kite-mcp-server/kc/watchlist#github.com/algo2go/kite-mcp-watchlist#g' {} \;
sed -i 's#^module github.com/zerodha/kite-mcp-server/kc/watchlist$#module github.com/algo2go/kite-mcp-watchlist#' go.mod
