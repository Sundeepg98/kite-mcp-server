...
find . -path ./kc/eventsourcing -prune -o -path ./.research -prune -o -name 'go.mod' -type f -print | while read -r mod; do
	if grep -qE 'github\.com/algo2go/kite-mcp-eventsourcing => ' "$mod" 2>/dev/null; then
...
git rm -rq kc/eventsourcing/

find . -path ./.research -prune -o -name 'go.mod' -type f -print | while read -r mod; do
	if grep -qE 'github\.com/algo2go/kite-mcp-eventsourcing v0\.0\.0-00010101000000-000000000000' "$mod" 2>/dev/null; then
