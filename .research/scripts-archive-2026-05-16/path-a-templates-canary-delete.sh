#!/usr/bin/env bash
# Path A.8' — kc/templates Phase B canary deletion ON MASTER.
...
grep -nE 'algo2go/kite-mcp-templates' go.mod | head -3
...
find . -path ./kc/templates -prune -o -path ./.research -prune -o -name 'go.mod' -type f -print | while read -r mod; do
	if grep -qE 'github\.com/algo2go/kite-mcp-templates => ' "$mod" 2>/dev/null; then
...
echo ""
echo "=== Phase 3: Drop ./kc/templates from go.work (CRLF-safe) ==="
sed -i '/^[[:space:]]\+\.\/kc\/templates\b/d' go.work
echo "go.work member count: $(grep -cE '^[[:space:]]+\./' go.work)"

echo ""
echo "=== Phase 4: Drop kc/templates COPY line from Dockerfile ==="
sed -i '/^COPY kc\/templates\/go\.mod kc\/templates\/go\.sum/d' Dockerfile
echo "Dockerfile templates lines: $(grep -c 'kc/templates' Dockerfile || true)"

echo ""
echo "=== Phase 5: Delete ./kc/templates/ ==="
git rm -rq kc/templates/
...
find . -path ./.research -prune -o -name 'go.mod' -type f -print | while read -r mod; do
	if grep -qE 'github\.com/algo2go/kite-mcp-templates v0\.0\.0-00010101000000-000000000000' "$mod" 2>/dev/null; then
