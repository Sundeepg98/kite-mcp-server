#!/usr/bin/env bash
# Path A.10 — Phase B viability test for kc/domain in scratch.

set -euo pipefail

SCRATCH=/tmp/domain-phase-b-test
...
echo "Step 2a: drop replace algo2go/kite-mcp-domain in root go.mod"
...
echo "Step 2c: drop ./kc/domain from go.work (CRLF-safe \\b)"
sed -i '/^[[:space:]]\+\.\/kc\/domain\b/d' go.work
echo "go.work member count: $(grep -cE '^[[:space:]]+\./' go.work)"

echo "Step 2d: rename ./kc/domain out of the way"
mv kc/domain kc/domain.MOVED-FOR-PHASE-B-TEST

echo "Step 2e: drop kc/domain COPY line from Dockerfile"
