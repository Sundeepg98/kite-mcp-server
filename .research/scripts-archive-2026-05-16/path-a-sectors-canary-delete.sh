...
sed -i '/github\.com\/algo2go\/kite-mcp-sectors => \.\/kc\/sectors/d' go.mod

find . -path ./kc/sectors -prune -o -path ./.research -prune -o -name 'go.mod' -type f -print | while read -r mod; do
	if grep -qE 'github\.com/algo2go/kite-mcp-sectors => ' "$mod" 2>/dev/null; then
		sed -i '/github\.com\/algo2go\/kite-mcp-sectors => /d' "$mod"
		echo "  patched: $mod"
	fi
done

sed -i '/^[[:space:]]\+\.\/kc\/sectors\b/d' go.work
echo "go.work members: $(grep -cE '^[[:space:]]+\./' go.work)"

sed -i '/^COPY kc\/sectors\/go\.mod kc\/sectors\/go\.sum/d' Dockerfile

git rm -rq kc/sectors/

find . -path ./.research -prune -o -name 'go.mod' -type f -print | while read -r mod; do
	if grep -qE 'github\.com/algo2go/kite-mcp-sectors v0\.0\.0-00010101000000-000000000000' "$mod" 2>/dev/null; then
