...
- **RiskGuard**: order-placing tools go through the RiskGuard middleware (`algo2go/kite-mcp-riskguard/`). It can reject with `global freeze`, `user freeze`, `daily limit`, `duplicate order`, or `rate limit`. Return the error as-is — don't swallow it.
