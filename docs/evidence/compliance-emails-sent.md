...
5. **Never send legal / compliance email from a non-project address.** Use the canonical project email (currently `g.karthick.renusharmafoundation@gmail.com` per CLAUDE.md global) so the thread is findable.
