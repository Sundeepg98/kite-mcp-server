...
| R-06 | SEBI regulatory action (algorithmic trading without registration) | Medium | Critical | `ENABLE_TRADING=false` on hosted Fly.io instance (Path 2 read-only); local self-hosted preserves trading via personal-use safe harbor (OpenAlgo precedent); `algo2go/kite-mcp-riskguard/otr_band.go` enforces SEBI Feb-2026 OTR exemption bands | Medium — first regulator inquiry could force feature gate or shutdown of hosted; local fork persists | maintainer |
